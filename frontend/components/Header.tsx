...
import { productsAPI, bundlesAPI, type Product, type Bundle } from "@/lib/api";
        ...
        <>
          ...
          {filteredBundles.map((bundle) => {
            const productNames = bundle.products
              .map((bp) => `${bp.quantity}x ${bp.product?.name || ""}`)
              .filter(Boolean)
              .join(", ");
      ...
      ? bundles.filter((b) => {
          ...
          return (
            ...
            b.products.some(
              (bp) =>
                bp.product &&
                (bp.product.name.toLowerCase().includes(lq) ||
                  bp.product.description?.toLowerCase().includes(lq))
            )
          );
        })
      : [];
